...
use App\Models\TenatPropertyUtility;
use Carbon\Carbon;
            ...
            if ($currentDate->lessThanOrEqualTo($leaseEndDate)) {
                ...
                if($leasePartners->count() >0){
                    ...
                    if($default_partner->is_gst == '1'){
                        ...
                        $cam_paid_amount = $cam_total_amount;
                    }
                    ...
                    $addUtilityInvoice->invoice_no = $invoice_no;
                    ...
                    if($addUtilityInvoice) {
                        ...
                        $extrautilityTotal = 0;
                        /*-------Electricity------------------------------------------*/
                        $electricityInvoice = TenatPropertyUtility::where('property_id',$lease->property_id)
                        ->where('tenant_id',$lease->tenant_id)
                        ->whereMonth('bill_date', $billDate->format('m'))
                        ->whereYear('bill_date', $billDate->format('Y'))
                        ->first();
                        
                        if($electricityInvoice){
                            $final_amount = $electricityInvoice->final_amount;
                            $utilityTotal += $final_amount;
                            $electricityInvoice = new InvoiceDetail;
                            $electricityInvoice->invoice_id = $addUtilityInvoice->id;
                            $electricityInvoice->random_id = $random_no;
                            $electricityInvoice->item_desc = 'Electricity Bill at Actual '.$bmonth.' '.$bcurrentYear.'';
                            $electricityInvoice->rate = $final_amount;
                            $electricityInvoice->amount = $final_amount;
                            $electricityInvoice->sub_total = $final_amount;
                            $electricityInvoice->type = 'utility';
