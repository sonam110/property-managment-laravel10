                        ...
                        <form id="unitForm">
                            ...
                            <div class="col-sm-6 onlyforcommercial" id="onlyforcommercial">
                                <div class="mb-3">
                                    ...
                                    {{ Form::number('bath_rooms', null, ['class' => 'form-control', 'id'=>'bath_rooms bath_rooms','placeholder' => __('Bath Rooms')]) }}
                                </div>
...
@section('extrajs')  
    ...
    function openUnitModal(inputId = '') {
        ...
        if (inputId) {
            var newInput = inputId.split('_')[1];
            let unitName = $(`#${inputId}`).val();
            let radioInput = $(`#radioInput_${newInput}`).val();
            let unitFloorInput = $(`#unitFloorInput_${newInput}`).val();
            let rentAmountInput = $(`#rentAmountInput_${newInput}`).val();
            let unitTypeInput = $(`#unitTypeInput_${newInput}`).val();
            let bedRoomInput = $(`#bedRoomInput_${newInput}`).val();
            let bathRoomInput = $(`#bathRoomInput_${newInput}`).val();
            let totalRoomInput = $(`#totalRoomInput_${newInput}`).val();
            let squareRoomInput = $(`#squareRoomInput_${newInput}`).val();
            $('#unitName').val(unitName);
            $('.unit').val(radioInput);
            $('#unit_floor').val(unitFloorInput);
            $('#rent_amount').val(rentAmountInput);
            $('#unit_type').val(unitTypeInput);
            $('#bed_rooms').val(bedRoomInput);
            $('#bath_rooms').val(bathRoomInput);
            $('#total_rooms').val(totalRoomInput);
            $('#square_foot').val(totalRoomInput);
    ...
    $('#continueButton').on('click', function() {
        ...
        let unit = $('.unit').val();
        let unit_floor = $('#unit_floor').val();
        let rent_amount = $('#rent_amount').val();
        let unit_type = $('#unit_type').val();
        let bed_rooms = $('#bed_rooms').val();
        let bath_rooms = $('#bath_rooms').val();
        let total_rooms = $('#total_rooms').val();
        let square_foot = $('#square_foot').val();
        if (currentInputId) {
              // For regular input action, update the existing unit
              $(`#${currentInputId}`).val(unitName);
              $(`#radioInput_${unitCount}`).val(unit);
              $(`#unitFloorInput_${unitCount}`).val(unit);
              $(`#rentAmountInput_${unitCount}`).val(rent_amount);
              $(`#unitTypeInput_${unitCount}`).val(unit_type);
              $(`#bedRoomInput_${unitCount}`).val(bed_rooms);
              $(`#bathRoomInput_${unitCount}`).val(bath_rooms);
              $(`#totalRoomInput_${unitCount}`).val(total_rooms);
              $(`#squareRoomInput_${unitCount}`).val(square_foot);
    ...
    function addUnitRow(unitName = '', unit='', rent_amount='',unit_floor='', unit_type='',bed_rooms='',bath_rooms='',total_rooms='',square_foot='') {
        unitCount++;
        let unitRow = `
    ...
    $('#unitsContainer').on('click', '.copy-btn', function() {
            let inputId = unitCount;
            alert(inputId);
            let unitName = $(`#unitInput_${inputId}`).val();
            console.log(inputId);
            console.log(unitName);
            
            let unit = $(`radioInput_#${inputId}`).val();
            let unit_floor = $(`unitFloorInput_#${inputId}`).val();
            let rent_amount = $(`#rentAmountInput_${inputId}`).val();
            let unit_type = $(`#unitTypeInput_${inputId}`).val();
            let bed_rooms = $(`#bedRoomInput_${inputId}`).val();
            let bath_rooms = $(`#bathRoomInput_${inputId}`).val();
            let total_rooms = $(`#totalRoomInput_${inputId}`).val();
            let square_foot = $(`#squareRoomInput_${inputId}`).val();
            addUnitRow(unitName,unit,unit_floor,rent_amount,unit_type,bed_rooms,bath_rooms,total_rooms,square_foot);
